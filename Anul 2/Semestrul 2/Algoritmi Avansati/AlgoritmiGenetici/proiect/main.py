...
def binary_search(probSelectie, left, right, x):
    while left <= right:
        ...
        if probSelectie[mid-1] <= x <= probSelectie[mid]:
            return mid
...
# Pentru matricea de cromozomi, se creaza o lista vida pentru fiecare pozitie posibila in cromozom. In functie de
...
for i in range(1, etape + 1):
    ...
    # de recombinare.
    cromRecomb = [matrix[indice_adaugat]]  # va contine cromozomii selectati pt recombinare
    for j in range(dimPopulatie-1):
        u = random.random()
        indicec = binary_search(sumeF2, 0, dimPopulatie - 1, u)
        if i == 1:
    ...
    cromParticipanti = []
    for j in range(1, len(cromRecomb)):
        u = random.random()
