...
class Graf:
    ...
    def valideaza(self):
        ...
        nrInv = 0
        for i1, elem1 in enumerate(matriceDesfasurata):
            for i2, elem2 in enumerate(matriceDesfasurata[i1+1:]):
    ...
    def succesori(self, nod):
        ...
        for d in directii:
            ...
            if 0 <= lPlacuta <=2 and 0 <= cPlacuta <= 2:
                ...
                stareNoua[lGol][cGol], stareNoua[lPlacuta][cPlacuta] = stareNoua[lPlacuta][cPlacuta], stareNoua[lGol][cGol]
...
f = open('input.txt', 'r')
start = [[int(x) for x in ls.strip().split(" ")] for ls in f.readlines()]
print(start)
