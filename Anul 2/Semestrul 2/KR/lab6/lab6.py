...
class InfoJoc:
    ...
    def __init__(self, tabla=None):
        if tabla:
            ...
            self.matr = [['#' for i in range(3)] for i in range(3)]
        ...
        for i in range(InfoJoc.NR_COLOANE):
            for j in range(InfoJoc.NR_COLOANE):
                if self.matr[i][j] == InfoJoc.GOL:
                    ...
                    l_matrice.append(InfoJoc(matrNoua))
    ...
    def linie_deschisa(self, lista, jucator):
        return self.jucator_opus(jucator) not in lista
...
def min_max(stare):
    ...
    mutariExpandate = [min_max(mutare) for mutare in stare.mutari_posibile]
...
def alpha_beta(alpha, beta, stare):
    if stare.tabla_joc.final() or stare.adancime == 0 or alpha >= beta:
        stare.estimare = stare.tabla_joc.estimeaza_scor(stare.adancime)
        return stare
    ...
    if stare.j_curent == InfoJoc.JMAX:
        ...
        for mutare in stare.mutari_posibile:
            mutareExpandata = alpha_beta(alpha, beta, mutare)
            if scorMaxim < mutareExpandata.estimare:
                scorMaxim = mutareExpandata.estimare
                stare.stare_aleasa = mutareExpandata
        ...
        for mutare in stare.mutari_posibile:
            mutareExpandata = alpha_beta(alpha, beta, mutare)
            if scorMinim > mutareExpandata.estimare:
                scorMinim = mutareExpandata.estimare
                stare.stare_aleasa = mutareExpandata
